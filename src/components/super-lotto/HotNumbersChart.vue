        ...
        <div class="bar-chart">
          <div class="chart-bars">
            ...
            >
              ...
              <div class="bar-info">
                <span class="hot-score">热值: {{ item.hot_score.toFixed(2) }}</span>
                <span v-if="item.last_seen" class="last-seen">
                  最后: {{ formatDate(item.last_seen) }}
                </span>
              </div>
            </div>
          </div>
...
const averageFrequency = computed(() => {
  ...
  const sum = chartData.value.reduce((acc, n) => acc + n.frequency, 0)
